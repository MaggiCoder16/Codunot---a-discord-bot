...
async def edit_image(image_bytes: bytes, prompt: str = "", steps: int = DEFAULT_STEPS) -> bytes:
    ...
    payload = {"model": FLUX_MODEL, "prompt": prompt.strip() or "high quality image edit", "steps": steps, "seed": seed, "webhook_url": WEBHOOK_URL}
...
async def merge_images(images: list[bytes], prompt: str = "", steps: int = DEFAULT_STEPS) -> bytes:
    ...
    headers = {"Authorization": f"Bearer {DEAPI_API_KEY}", "Accept": "application/json"}
    payload = {"model": FLUX_MODEL, "prompt": prompt.strip() or "A high quality merge of these subjects", "steps": steps, "seed": seed, "webhook_url": WEBHOOK_URL}
